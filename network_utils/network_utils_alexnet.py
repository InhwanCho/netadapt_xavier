...
class networkUtils_alexnet(NetworkUtilsAbstract):
    ...
    def __init__(self, model, input_data_shape, dataset_path, dataset, finetune_lr=1e-3,):
        ...
        if dataset == 'cifar10':
            ...
            train_dataset = datasets.folder.ImageFolder(root='/home/keti/Dataset/imagenet/train/', 
            transform=transforms.Compose([
                transforms.RandomResizedCrop(224),
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
                normalize,]))
            val_dataset = datasets.folder.ImageFolder(root='/home/keti/Dataset/imagenet/val/', 
            transform=transforms.Compose([
...
def alexnet(model, input_data_shape, dataset_path,dataset, finetune_lr=1e-3):
    print('dataset',dataset)
    return networkUtils_alexnet(model, input_data_shape, dataset_path, dataset, finetune_lr)