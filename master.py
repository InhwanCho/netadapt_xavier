...
def _launch_worker(worker_folder, model_path, block, resource_type, constraint, netadapt_iteration,
    ...
    with open(os.path.join(worker_folder,
        ...
        command_list = [sys.executable, _WORKER_PY_FILENAME, worker_folder, model_path, str(block), resource_type,
                        ...
                        lookup_table_path, dataset_path] + [str(e) for e in input_data_shape] + [model_arch] + [str(args.finetune_lr)]+[str(args.kd)] +[str(args.dataset)]
...
def master(args):
    ...
    if args.resume:
        ...
        network_utils = networkUtils.__dict__[model_arch](model, args.input_data_shape, args.dataset_path, args.dataset)
        ...
        network_def = network_utils.get_network_def_from_model(model) # helloworld conv(in_ch,out_ch) -> [(3, 16), (16, 32), (32, 64), (64, 10)]
        if args.lookup_table_path != None and not os.path.exists(args.lookup_table_path):
    ...
    while current_iter <= args.max_iters and current_resource > args.budget:
        ...
        ## init_resource_reduction = 0.025*(pretrained model resource)
        ### target_resource = current_resource - init_resource_reduction * (0.96 ** (current_iter - 1)) 
        #### budget = 0.025*(pretrained model resource)
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('-kd', type = str, default=None, 
                            help = ' shor-term fine-tune knowledged distillation')
    arg_parser.add_argument('--dataset', type=str, default='cifar10', help='dataset cifar10 or cifar100')
