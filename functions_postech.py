...
def get_keys_from_ordered_dict(ordered_dict):
    ...
    for key, _ in ordered_dict.items():
        if key == 'last_layer.0.weight':
            last_keys.append(key)
        elif key == 'last_layer.1.weight':
            last_keys.append(key)
        elif key == 'last_layer.1.bias':
            last_keys.append(key)
        elif key == 'last_layer.1.running_mean':
            last_keys.append(key)
        elif key == 'last_layer.1.running_var':
            last_keys.append(key)
        elif key == 'last_layer.1.num_batches_tracked':
            last_keys.append(key)
...
def measure_latency(model, input_data_shape, runtimes=200):
    '''
    ...
    for i in range(runtimes):       
        if is_cuda:
            ...
            with torch.no_grad():
                ...
                model(input)
                # finish = time.time()
        total_time_1 += start_1.elapsed_time(end)
        
        # total_time += (finish - start)
    # print(total_time_1)
    # print(total_time)
    return total_time_1/float(runtimes)
...
def simplify_model_based_on_network_def(simplified_network_def, model):
        ...
        simplified_state_dict = get_keys_from_ordered_dict(simplified_state_dict)
        #print('simplified_model.keys',simplified_state_dict)
        kept_filter_idx = None

        for layer_param_full_name in simplified_state_dict:
            ...
            if kept_filter_idx is None:
                ...
                if layer_param_name == WEIGHTSTRING: #WEIGHTSTRING == layer_param_name:                    
                    ...
                    # print("layer : {}, layer_param_name : {}, kept_filter_idx :{}".format(layer,
                    #       layer_param_name, kept_filter_idx.dtype))
                    print('    simplify_model> simplify Conv layer {}: output channel biases {}'.format(layer_name_str, 
                          len(kept_filter_idx)))
                    setattr(layer, layer_param_name,
                            torch.nn.Parameter(getattr(layer, layer_param_name)[kept_filter_idx.long()])) #####re
                ...
                if any(substr == layer_param_name for substr in [WEIGHTSTRING, BIASSTRING]):
                    setattr(layer, layer_param_name,
                            torch.nn.Parameter(getattr(layer, layer_param_name)[kept_filter_idx.long()], requires_grad=True)) ###re
                    ...
                    setattr(layer, layer_param_name,
                            torch.nn.Parameter(getattr(layer, layer_param_name)[kept_filter_idx.long()], requires_grad=False)) ###re
                    layer.num_features = len(kept_filter_idx)
                    print('    simplify_model> simplify {} layer {}: {} {}'.format(layer_type_str,
                          layer_name_str, layer_param_name, layer.num_features))
